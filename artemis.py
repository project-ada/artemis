...
class Artemis(object):
	...
	def __kubectl(self, cmd):
		return subprocess.check_output("%s %s" % (self.config.get('kubectl'), cmd), shell=True)
	...
	def run_cli(self):
		...
		if sys.argv[1] == 'get-image-tag':
			...
			print "Tag for %s in %s: %s" % (comp.get_name(), env.get_name(), comp.get_image_tag())
...
class Environment(object):
	...
	def __gen_component(self, file_path):
		...
		return Component(name=file_name.split("/")[-1] if '/' in file_name else file_name, file=file_path, component_type='kube' if file_extension == '.yaml' else 'tf')
